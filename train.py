...
def main():
    ...
    for iteration in range(cfg['train_iterations']):
        ...
        for step in steps:
            ...
            plot_loss += torch.sum(weighted, dim=1).detach().numpy()  # sum over batch
        ...
        # Compute observation prediction accuracy
        with torch.no_grad():
            # Average MSE across all steps and batch elements
            obs_mses = []
            for step in steps:
                # x_gen[2] is (mu, sigma) from the generative path (g_gen -> p -> x)
                mu_pred = step.x_gen[2][0]
                obs_mses.append(torch.mean((mu_pred - step.obs) ** 2).item())
            avg_obs_mse = np.mean(obs_mses)

            # Also track per-prediction-path MSEs
            mse_from_p = np.mean([torch.mean((s.x_gen[0][0] - s.obs) ** 2).item() for s in steps])
            mse_from_g_inf = np.mean([torch.mean((s.x_gen[1][0] - s.obs) ** 2).item() for s in steps])
            mse_from_g_gen = np.mean([torch.mean((s.x_gen[2][0] - s.obs) ** 2).item() for s in steps])
